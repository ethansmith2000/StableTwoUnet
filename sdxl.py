...
class TwoUnetsXLPipeline(StableDiffusionXLPipeline):
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if (1-t.item()/999.0) > swap_step:
                    latent_model_input = torch.cat([latents, latents], dim=0)
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                    added_cond_kwargs = {"text_embeds": torch.cat([negative_add_text_embeds,add_text_embeds,]), "time_ids": torch.cat([negative_add_time_ids,add_time_ids,])}
                    if ip_adapter_image is not None:
                    ...
                    noise_pred_uncond = self.uncond_unet(
                        latent_model_input,
                        t,
                ...
                if perturb_factor > 0.0:
                    noise_pred_uncond = noise_pred_uncond + perturb_factor * torch.randn_like(noise_pred_uncond)
                ##############
